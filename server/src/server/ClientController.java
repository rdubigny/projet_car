...
public class ClientController implements Runnable {
    ...
    private void readFile(String fileName) {
        System.out.println("read file: " + login + "/" + fileName);
        Archive archive = Server.dataNode.memory.mem.get(login + "/" + fileName);
        archive.setFileName(archive.getFileName().replace(login+"/", ""));
        messenger.send(new DataContainer("FILE", archive));
    }

    private void writeFile(Data data) {
        ArchiveAndList archiveList = (ArchiveAndList) data;
        archiveList.archive.setFileName(login + "/" + archiveList.archive.getFileName());
        Server.dataNode.writeMultiple((Data) archiveList);
        messenger.close();
    }
    ...
    private void eraseFile(String fileName) {
        ...
        if (archive == null) {
            ...
            messenger.send(archive.getFileName() + " erased from the memory");
        }
    }
    ...
    private void registerFile(String parameter) {
        if (parameter == null) {
            ...
            IdList idList;
            idList = Server.nameNodeManager.register(login + "/" + parameter);
            if (idList == null) {
    ...
    private void locateFile(String parameter) {
        if (Config.getInstance().IamTheMaster()) {
            IdList idList;
            idList = Server.nameNode.getIds(login + "/" + parameter);
            if (idList == null) {
    ...
    private void unregisterFile(String parameter) {
        if (Config.getInstance().IamTheMaster()) {
            Server.nameNodeManager.unregister(login + "/" + parameter);
            messenger.send("OK");
        } else {
            messenger.send("Internal Error");
        }
